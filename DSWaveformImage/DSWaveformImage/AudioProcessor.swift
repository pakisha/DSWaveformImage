...
extension AudioProcessor {
    ...
    fileprivate func extract(samplesFrom assetReader: AVAssetReader, downsampledTo targetSampleCount: Int) -> [Float] {
        ...
        while assetReader.status == .reading {
            ...
            if let sampleBuffer = trackOutput.copyNextSampleBuffer(), let blockBuffer = CMSampleBufferGetDataBuffer(sampleBuffer) {
        ...
        // rucno downsamplovanje
        if targetSampleCount < outputSamples.count && outputSamples.count > 0 {
            // koliko vrednosti iz outputa se pretvara u jednu vrednost u paddedSamples
            let step = outputSamples.count/targetSampleCount
            
            // sabira step broj vrednosti iz outputsamples u jednu vrednost u paddedSamples
            for index in 0..<paddedSamples.count {
                var sum: Float = 0.0
                for i in 0..<step {
                    sum += outputSamples[index*step + i]
                }
                paddedSamples[index] = sum/Float(step)
            }
            // rucno upsamplovanje
        } else if targetSampleCount > outputSamples.count && outputSamples.count > 0 {
            let start = (targetSampleCount - outputSamples.count)/2
            paddedSamples.replaceSubrange(start..<outputSamples.count, with: outputSamples)
            paddedSamples.removeSubrange(targetSampleCount ..< targetSampleCount + start)
        } else {
            // stari nacin punjenja paddedSamples, ovo je pravilo problem zato sto se desavalo da paddedSamples ne bude velicine targetSampleCount
    ...
    private func process(_ samples: UnsafeMutablePointer<Int16>,
        ...
        //        var samplesPerPixel = max(1, sampleCount(from: assetReader) / targetSampleCount)
        //
        ////        if sampleLength < samplesPerPixel {
        ////            samplesPerPixel = 1
        ////        }
        //
